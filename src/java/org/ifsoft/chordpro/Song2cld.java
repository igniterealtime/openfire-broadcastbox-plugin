...
class Song2cld {
	...
	if (t.isEmpty() || ((slash = t.indexOf("/")) == -1)) {
	    System.err.println("Check time signature syntax (no '/')") ;
	    return ;
	}
	String num = t.substring(0, slash) ;
	String den = t.substring(slash+1) ;
	if (num.isEmpty() || den.isEmpty()) {
	    System.err.println("Check time signature syntax (num/den)") ;
	    return ;
	}
	try {
	    ...
	    System.err.println("Check time signature syntax (not integer)") ;
	    return ;
	}
	...
	if (isPowerOfTwo(division)) {
	    ...
	    default:
		System.err.println("Time sig denomination is >64") ;
		division = 64 ;
		ticksPerDivision = resolution/16 ;
	    }
	    ticksPerBar = divisionsPerBar * ticksPerDivision ;
	} else {
	    System.err.println("Check time signature (division is not power of 2)") ;
	}
    }
	...
	// System.out.print(e.elementDirective + " ") ;
	// System.out.println("'" + e.elementValue + "'") ;
	...
	if (dir.equals("title") || dir.equals("t")) {
		...
		System.err.println("Couldn't add title info") ;
	    }
		...
		*/
		...
		System.err.println("Couldn't add page break (soc)") ;
	    }
		...
		*/
		...
		System.err.println("Couldn't add page break (soi)") ;
	    }
		...
		*/
		...
		System.err.println("Couldn't add page break (chorus)") ;
	    }
		...
		System.err.println("Couldn't add artist info") ;
	    }
		...
		System.err.println("Couldn't add composer info") ;
	    }
		...
		System.err.println("Couldn't add copyright info") ;
	    }
		...
		System.err.println("Couldn't add key signature") ;
	    }
		...
		m.addTimeSignature(divisionsPerBar, division,
				   (timeStamp + tickDelay)) ;
		// System.out.print("Div: " + division) ;
		// System.out.println(" PerBar: " + divisionsPerBar) ;
	    } catch(Exception exc) {
		System.err.println("Couldn't add time signature") ;
	    }
		...
		System.err.println("Couldn't add tempo (BPM)") ;
	    }
		...
		System.err.println("Couldn't add accompaniment style") ;
	    }
		...
		System.err.println("Couldn't add start accompaniment") ;
	    }
		...
		System.err.println("Couldn't add stop accompaniment") ;
	    }
	} else {
	...
	*/
	...
	if (! tempoFlag) {
	    System.err.println("Tempo directive is needed before 1st chord") ;
	    // Assume 120 BPM
	    try {
		// Tempo could change in the middle of a song
		tempo = "120" ;
		...
		System.err.println("Couldn't add tempo (pre)") ;
	    }
	}
	if (! timeFlag) {
	    System.err.println("Time directive is needed before 1st chord") ;
	    // Assume 4/4
	    timeSignature = "4/4" ;
	    convertTimeSignature(timeSignature, m.getResolution()) ;
	    try {
		...
		System.err.println("Couldn't add time signature (pre)") ;
	    }
	}
	if (! keyFlag) {
	    System.err.println("Key directive is needed before 1st chord") ;
	    // Assume C major
	    convertKey("C") ;
	    try {
		m.addKeySignature(key, keyScale, (long)0) ;
	    } catch(Exception exc) {
		System.err.println("Couldn't add key signature (pre)") ;
	    }
	}
	if (! stylecodeFlag) {
	    System.err.println("Stylecode directive is needed before 1st chord") ;
		...
		System.err.println("Couldn't add accompaniment style (pre)") ;
	    }
	}
	...
	*/
	...
	try {
	    ...
	    */
	    ...
	    System.err.println("Couldn't add chord") ;
	}
	...
	if (instrumentalSection) {
		...
		System.err.println("Couldn't add blank lyric for a chord") ;
	    }
	}
	*/
	...
	if (e.elementText.charAt(0) != '*') {
	    System.err.println("*First char in annotation is not '*'") ;
	    return ;
	}
	...
	try {
	    ...
	    System.err.println("Could not add section control '" + ann + "'") ;
	    return ;
	}
    }
	...
	// TBD stub
	// System.out.println("'" + e.elementTab + "'") ;
    }
	...
	try {
	    ...
	    */
	    ...
	    System.err.println("*fatal* Could not add lyric line break") ;
	    return ;
	}
    }
	...
	try {
	    ...
	    */
	    ...
	    System.err.println("*fatal* Could not add MIDI boilerplate") ;
	    return ;
	}
	...
	try {
	    long finalTimestamp = timeStamp + tickDelay ;
	    /*
	    midiFile.addAccompStop(finalTimestamp) ;
	    */
	    midiFile.addEndOfTrack(finalTimestamp) ;
	} catch(Exception exception) {
	    System.err.println("*fatal* Could not add MIDI end of track") ;
	    return ;
	}
    }
}
